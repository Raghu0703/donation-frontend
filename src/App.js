...
function App() {
  ...
  // Live logs — start empty, populated by actual submissions
  ...
  const handleNavClick = (portal) => {
    if (portal === 'home') { setActivePortal('home'); return; }
    if (loggedIn[portal]) { setActivePortal(portal); } else { setLoginTarget(portal); }
  };
  ...
  return (
    <div className="App">
      ...
      <main className="main-content">
        {/* Donor Portal */}
        {activePortal === 'donor' && loggedIn.donor && (
          <div className="portal-container">
            ...
            {/* Donor's own submission history */}
            {donorLog.length > 0 && (
              <div className="donation-form-container" style={{ marginTop: '2rem' }}>
                ...
                <div style={{ overflowX: 'auto' }}>
                  <table style={styles.table}>
                    <thead>
                      <tr>
                        {['Item', 'Qty', 'Phone', 'Address', 'Date'].map(h => <th key={h} style={styles.th}>{h}</th>)}
                      </tr>
                    </thead>
                    <tbody>
                      {donorLog.filter(r => r.donor === formData.name || true).map((row, i) => (
        ...
        {/* Receiver Portal */}
        {activePortal === 'receiver' && loggedIn.receiver && (
          <div className="portal-container">
            <div className="portal-header">
            ...
            {selectedCategory && (
              <div className="donation-form-container" style={{ marginTop: '2rem' }}>
                <form onSubmit={handleRequestSubmit} className="premium-form">
                  ...
                  <button type="submit" className="form-submit" style={{ background: 'linear-gradient(45deg, #4285f4, #1a73e8)' }}>
                    Submit Request 📋
                  </button>
                </form>
              </div>
            ...
            {/* Receiver's own request history */}
              ...
              <div className="donation-form-container" style={{ marginTop: '2rem' }}>
                ...
                <div style={{ overflowX: 'auto' }}>
                  <table style={styles.table}>
                    <thead>
                      <tr>
                        {['Item', 'Qty', 'Phone', 'Reason/Address', 'Date'].map(h => <th key={h} style={styles.th}>{h}</th>)}
                      </tr>
                    </thead>
                    <tbody>
        ...
        {/* Admin Dashboard */}
        {activePortal === 'admin' && loggedIn.admin && (
          <div className="portal-container">
            <div className="portal-header">
            ...
            {/* LIVE DONATION ANALYSIS */}
            <div className="chart-card" style={{ marginBottom: '2rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
                <h3 style={{ margin: 0 }}>📊 Live Donation Submissions</h3>
                <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
                  <div style={styles.miniStat}>
                    <span style={styles.miniVal}>{totalDonated}</span>
                    <span style={styles.miniLabel}>Total Units</span>
                  </div>
                  <div style={styles.miniStat}>
                    <span style={styles.miniVal}>{new Set(donorLog.map(r => r.donor)).size}</span>
                    <span style={styles.miniLabel}>Donors</span>
                  </div>
                  <div style={styles.miniStat}>
                    <span style={styles.miniVal}>{donorLog.length}</span>
                    <span style={styles.miniLabel}>Submissions</span>
                  </div>
                </div>
              </div>
              {donorLog.length === 0 ? (
                <div style={styles.emptyState}>
                  ...
                  <p>No donations submitted yet. Once donors submit, their entries will appear here live.</p>
                </div>
              ) : (
                <div style={{ overflowX: 'auto' }}>
                  <table style={styles.table}>
                    <thead>
                      <tr>
                        {['#', 'Donor Name', 'Item', 'Qty', 'Phone', 'Pickup Address', 'Date'].map(h => <th key={h} style={styles.th}>{h}</th>)}
                      </tr>
                    </thead>
                    <tbody>
                      {donorLog.map((row, i) => (
            ...
            {/* LIVE RECEIVER ANALYSIS */}
            <div className="chart-card">
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
                <h3 style={{ margin: 0 }}>📋 Live Receiver Requests</h3>
                <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
                  <div style={styles.miniStat}>
                    <span style={styles.miniVal}>{totalRequested}</span>
                    <span style={styles.miniLabel}>Total Units</span>
                  </div>
                  <div style={styles.miniStat}>
                    <span style={styles.miniVal}>{new Set(receiverLog.map(r => r.receiver)).size}</span>
                    <span style={styles.miniLabel}>Receivers</span>
                  </div>
                  <div style={styles.miniStat}>
                    <span style={styles.miniVal}>{receiverLog.length}</span>
                    <span style={styles.miniLabel}>Requests</span>
                  </div>
                </div>
              </div>
              {receiverLog.length === 0 ? (
                <div style={styles.emptyState}>
                  <div style={{ fontSize: '3rem', marginBottom: '1rem' }}>📭</div>
                  <p>No requests submitted yet. Once receivers submit, their entries will appear here live.</p>
                </div>
              ) : (
                <div style={{ overflowX: 'auto' }}>
                  <table style={styles.table}>
                    <thead>
                      <tr>
                        {['#', 'Receiver Name', 'Category', 'Qty', 'Phone', 'Reason / Address', 'Date'].map(h => <th key={h} style={styles.th}>{h}</th>)}
                      </tr>
                    </thead>
                    <tbody>
                      {receiverLog.map((row, i) => (
...
const styles = {
  ...
  modal: {
    ...
    borderRadius: '24px', padding: '3rem', width: '100%', maxWidth: '440px',
  ...
  closeBtn: {
    ...
    fontSize: '1.2rem', borderRadius: '50%', width: '36px', height: '36px', cursor: 'pointer',
  },
  ...
  input: {
    width: '100%', padding: '1rem 1.25rem', marginBottom: '1rem', borderRadius: '14px',
    background: 'rgba(255,255,255,0.2)', border: '1px solid rgba(255,255,255,0.3)',
    color: 'white', fontSize: '1rem', outline: 'none', boxSizing: 'border-box',
  },
  loginBtn: {
    width: '100%', padding: '1.1rem', background: 'linear-gradient(45deg, #00c851, #007e33)',
    color: 'white', border: 'none', borderRadius: '14px', fontSize: '1.1rem',
    fontWeight: 800, cursor: 'pointer', marginTop: '0.5rem', letterSpacing: '0.5px',
    boxShadow: '0 8px 24px rgba(0,200,81,0.4)',
  },
  ...
  logoutBtn: {
    ...
    color: 'white', borderRadius: '50px', cursor: 'pointer', fontWeight: 600, fontSize: '0.9rem',
  },
  table: { width: '100%', borderCollapse: 'collapse', color: 'white' },
  th: {
    padding: '0.8rem 1rem', textAlign: 'left', background: 'rgba(255,255,255,0.15)',
    fontWeight: 700, fontSize: '0.95rem', borderBottom: '1px solid rgba(255,255,255,0.2)',
  },
  ...
  badge: {
    ...
    padding: '0.2rem 0.75rem', borderRadius: '20px', fontWeight: 700, fontSize: '0.85rem',
  },
  miniStat: {
    background: 'rgba(255,255,255,0.1)', borderRadius: '14px', padding: '0.75rem 1.25rem',
  ...
  emptyState: {
    textAlign: 'center', padding: '3rem', color: 'rgba(255,255,255,0.6)',
    background: 'rgba(255,255,255,0.05)', borderRadius: '16px',
    border: '2px dashed rgba(255,255,255,0.2)', fontSize: '1.1rem',
  },
};
